...
class Torpedo():
    def __init__(self, window, loc, listPathTopedoA, pathImages, isHitTheTarget, spf=0): # spf: Time to display 1 image
        ...
        self.__image = pygame.image.load(pathImages[0]).convert_alpha() if isHitTheTarget == 1 else (pygame.image.load(pathImages[1]).convert_alpha() if isHitTheTarget == 0 else pygame.image.load(pathImages[2]).convert_alpha())
