...
class Ship():
    def __init__(self, window, loc, path, id, direction=0):
        self.window = window
        self.oldLoc = loc
        self.loc = loc
        self.id = id
        self.__image = pygame.transform.rotate(pygame.image.load(path).convert_alpha(), direction * 90)
    ...
    def rotate(self, leftOrRight): # True: right, False: left
        if leftOrRight:
            self.direction = (self.direction + 1) % len(MATRIX_ROTATE_RIGHT)
            self.__image = pygame.transform.rotate(self.__image, 90)
            self.loc = (self.loc[0] - MATRIX_ROTATE_RIGHT[self.direction][0]*(self.height - self.width), self.loc[1] + MATRIX_ROTATE_RIGHT[self.direction][1]*(self.height - self.width))
            self.width, self.height = self.height, self.width
            self.updateHitBox()
        else:
            if self.direction - 1 < 0: self.direction = len(MATRIX_ROTATE_LEFT) - 1
            else: self.direction -= 1
            self.__image = pygame.transform.rotate(self.__image, 270)
            self.loc = (self.loc[0] - MATRIX_ROTATE_LEFT[self.direction][0]*(self.height - self.width), self.loc[1] + MATRIX_ROTATE_LEFT[self.direction][1]*(self.height - self.width))
            self.width, self.height = self.height, self.width
            self.updateHitBox()
